...
asm_sincos:
    ...
    vabs.f32 s1, s0             @ s1 = abs(x)
    vldr s2, =0x3fc90fdb        @ s2 = pi/2
    vldr s3, =0x40490fdb        @ s3 = pi
    vcmp.f32 s1, s2             @ if (abs(x) > pi/2)
    vmrs apsr_nzcv, fpscr       @ {
    itt gt                      @   s1 = pi - abs(x)
    vsubgt.f32 s1, s3, s1       @   save quadrant bit in r0
    orrgt r0, #0x40000000       @ }
    vsub.f32 s0, s2, s1         @ s0 = pi/2 - x (to calculate cosine from sine)
